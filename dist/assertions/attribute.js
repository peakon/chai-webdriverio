...
var doesElementHaveAttribute = function doesElementHaveAttribute(element, attribute, expected) {
...
function attribute(client, chai, utils, options) {
    ...
    chai.Assertion.addMethod('attribute', function (attribute, expected) {
        ...
        this.assert(elementContainsAttribute.result, 'Expected an element matching <' + selectorOrElement + '> to contain attribute "' + attribute + '" with value "' + expected + '", but only found these values: ' + elementContainsAttribute.values, 'Expected an element matching <' + selectorOrElement + '> not to contain attribute "' + attribute + ' with value "' + expected + '", but found these attributes: ' + elementContainsAttribute.values);
    });
}